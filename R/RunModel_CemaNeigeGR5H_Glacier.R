RunModel_CemaNeigeGR5H_Glacier <- function(InputsModel, RunOptions, Param) {
  ...
  # Param GR5H: 5, Param CemaNeige 2, Param Glacier: 3
  ParamCemaNeige <- Param[6:7]
  NParamMod      <- 5L
  ...
  # Glacier Parameters
  if (length(Param) > 7L) {
    ParamGlacier <- Param[8:length(Param)]
  } else {
    ParamGlacier <- NULL
  }
